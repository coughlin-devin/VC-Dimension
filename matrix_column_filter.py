...
def shatter(n, k, vc, lsum, usum, lpc, upc):
    ...
    cm = S(n,k) # TODO OPTIMIZE calculate column sums and which columns those are from the construction of smaller communication matrix?
    ...
    for comb in combinations(filter_cmatrix(cm, n, lsum, usum, lpc, upc), vc):
        ...
        if len(array) == 2**vc:
            # TODO add other interesting fields for analysis
            ...
            if len(dfs) == 10: #NOTE arbitrary choice of stopping at 10 shattered sets, will likely be in order so very similar values
                return dfs
    return dfs
